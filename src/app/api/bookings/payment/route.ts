    ...
    if (bookingData.selectedAttractions && Array.isArray(bookingData.selectedAttractions) && bookingData.selectedAttractions.length > 0) {
      ...
      if (hasMedicalSelection) {
        ...
        if (attractionDate >= minAttractionDate && attractionDate < maxAttractionDate) {
          let currentAttractionDate = new Date(attractionDate);
          for (let i = 0; i < attractionsToProcess.length; i++) {
            const attraction = attractionsToProcess[i];
            if (i > 0 && i % 2 === 0) {
              currentAttractionDate = new Date(currentAttractionDate.getTime() + 24 * 60 * 60 * 1000);
              currentAttractionDate.setUTCHours(1, 0, 0, 0);
            } else if (i > 0 && i % 2 === 1) {
              currentAttractionDate.setUTCHours(6, 0, 0, 0);
            }
            if (currentAttractionDate >= maxAttractionDate) break;
            // 景点游览时间，默认2小时
